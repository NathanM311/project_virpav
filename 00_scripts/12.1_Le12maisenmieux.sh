...
# Traduction GVDB : On fouille les colonnes 5, 7 et 10 pour maximiser les chances !
awk -F'\t' 'NR>1 {
    code=$1; gsub(/m/, "", code);
    fonction=$5;
    if (fonction == "" || fonction == "-") fonction=$7;
    if (fonction == "" || fonction == "-") fonction=$10;
    
    # Nettoyage des caractères invisibles qui font planter les dictionnaires
    gsub(/\r/, "", fonction); 
    
    print code"\t"fonction
}' ${DB_DIR}/GVOGs/gvog.complete.annot.tsv >> $DICT_TEMP
